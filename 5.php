...
function sobrePeso($datos) {
    ...
    foreach ($datos as $perro) {
        ...
        if ($edadHumana >= 60 && $peso > 7) {
            $contador++;

        } elseif ($edadHumana < 40 && $peso > 3) {
            $contador++;

        } elseif ($edadHumana >= 40 && $edadHumana < 60 && $peso > 6) {
            $contador++;
        }
    }
...
$total = count($data);
$conSobrePeso = sobrePeso($data);
$porcentaje = ($conSobrePeso / $total) * 100;

echo "Perros con sobrepeso: $conSobrePeso de $total<br>";
echo "Porcentaje: " . round($porcentaje, 2) . "%";
